...
public class Exercises {
	...
	public boolean monkeyTrouble(boolean aSmile, boolean bSmile) {
		if ((aSmile && bSmile == false) || (aSmile == false && bSmile)) {
			return false;
		} else {
			return true;
		}
	...
	public int dateFashion(int you, int date) {
		if ((you <=2 || date <=2) || (you > 10 || date >10)) {
			return 0;
	...
	public boolean squirrelPlay(int temp, boolean isSummer) {
		if ((isSummer && temp >=60 && temp <=100) || (!isSummer && temp >=60
				&& temp <=90)) {
			return true;
		} else {
			return false;
    ...
    public String yourCakeAndEatItToo(double mealAmount, boolean isBirthday) {
		if ((isBirthday && mealAmount <= 5) || (!isBirthday && mealAmount <=10)) {
			return "standard";
		} else if ((isBirthday && mealAmount <=10) || (!isBirthday && mealAmount <=15)) {
			return "special";
		} else {
			return "ginormous";
		}
	...
	/*
	 35. Given 3 int values, a b c, return their sum. However, if one of the values
	 is 13 then it does not count towards the sum and values to its immediate right
	 do not count. So for example, if b is 13, then both
